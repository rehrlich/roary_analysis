...
# https://github.com/katholt/plotTree/blob/master/plotTree.R
...
plotTree<-function(tree,ladderise=NULL,heatmapData=NULL,
                   barData=NULL,infoFile=NULL,blockFile=NULL,
                   snpFile=NULL,gapChar="?",genome_size=5E6,blwd=5,
                   block_colour="black",snp_colour="red",
                   genome_offset=0,colourNodesBy=NULL,
                   infoCols=NULL,outputPDF=NULL,outputPNG=NULL,
                   w,h,heatmap.colours=rev(gray(seq(0,1,0.1))),
                   tip.labels=F,tipLabelSize=1,offset=0,tip.colour.cex=0.5,
                   legend=T,legend.pos="bottomleft",ancestral.reconstruction=F,
                   cluster=NULL,tipColours=NULL,lwd=1.5,axis=F,
                   axisPos=3,edge.color="black",infoCex=0.8,
                   colLabelCex=0.8,treeWidth=10,infoWidth=10,dataWidth=30,
                   ...
                   vlines.heatmap=NULL,vlines.heatmap.col="black",
  ...
  if (!is.null(heatmapData)) {
    ...
    scale.range = as.matrix(seq(min(y.ordered,na.rm=T), max(y.ordered,na.rm=T),
                                length.out=length(heatmap.colours)+1))
    scale.range = as.matrix(scale.range[-1])
#    return(scale.range)
    # scale for heatmap
    par(mar=c(2,0,0,2))
    ...
    scale.plot <- barplot(rep(1, length(heatmap.colours)), space=0,
                          col=heatmap.colours, border=F, yaxt="n"
                          )
    mtext(side=1, at=scale.plot, text=round(scale.range), las=3, cex=0.6)
#    image(scale.range,
#          col=heatmap.colours,yaxt="n", breaks=heatmapBreaks,axes=F)

#    axis(side = 1, at = scale.range / max(y.ordered,na.rm=T), labels = round(scale.range))
